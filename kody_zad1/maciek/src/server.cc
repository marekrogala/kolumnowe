...
class DoubleColumnServer : public ColumnServer {
 public:
  ...
  int GetDoubles(int number, double *destination) {
    if (debug) printf("SERVING %d to %d, zoom %d, normalcy %s\n",
    ...
    for (int i = 0; i < number; ++i) {
      ...
      //destination[i] = ((int)Generate()) % 100;
    }
    return number;
  }
...
class IntColumnServer : public ColumnServer {
 public:
  ...
  int GetInts(int number, int *destination) {
	  static int dupa = 0;
    if (debug) printf("SERVING %d to %d, normalcy %s\n",
           low_range_, high_range_, normal_ ? "TRUE" : "FALSE");
    number = Serve(number);
    for (int i = 0; i < number; ++i) {
    	//destination[i] = ((i % 100) + 100) % 100;
  //  	destination[i] = 1; //++dupa;
    	//destination[i] = 10;
    	destination[i] = Generate();
    //destination[i] = 5;
      //destination[i] = (((((int)Generate()) % 100) + 100) % 100) + 1;
      //cout << "SERVER " << destination[i] << endl;
    //  if (destination[i] % 2) destination[i] = 5;
    //  else destination[i] = 4;
    }
    return number;
  }
...
class BoolColumnServer : public ColumnServer {
 public:
  ...
  int GetByteBools(int number, bool *destination) {
    if (debug) printf("SERVING probability %d\n", probability_);
    number = Serve(number);
    for (int i = 0; i < number; ++i) {
    	//destination[i] = (bool)(rand() % 2);
      destination[i] = Generate();
    }
    return number;
  }
...
RealDataServer::RealDataServer(const vector<int> &column_types) {
	...
	int query_size = 300;
#else
	int query_size = 300000000;
#endif

	query_size = 300;
 // query_size = random(1000000, 10000000);
...
vector<int> ColumnMap(int query_id) {
  ...
  switch(query_id) {
    ...
    case 20: return CreateVector(1, 1, 3, 2);
    case 30: return CreateVector(1, 1, 2, 2);
    case 31: return CreateVector(1, 1, 2, 2);
    case 32: return CreateVector(1, 1, 2);
    default: assert(false);
  }
}

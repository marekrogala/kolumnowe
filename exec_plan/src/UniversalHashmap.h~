...
public:
	...
	vector<void*> get_result(int&size) {
		...
		int z = 0;
		for(typename tr1::unordered_map<K, V>::iterator i = map_.begin(); i != map_.end(); ++i) {
			res1[z] = i -> first;
			res2[z] = i -> second;
			++z;
		}
...
public:
	...
	void insert(void* key, void* value, void* value2, int rows) {
		bool* key_buff = static_cast<bool*>(key);
		V* value_buff = static_cast<V*>(value);
		V2* value_buff2 = static_cast<V2*>(value);
		for(int i = 0; i < rows; ++i) {
...
public:
	void insert(void* key, void* value1, void* value2, int rows) {
		...
		for(int i = 0; i < rows; ++i) {
			typename tr1::unordered_map<K, pair<V, V2> > :: iterator x = map_.find(key_buff[i]);
			if (x == map_.end()) {
				map_[key_buff[i]] = make_pair(value_buff[i], value_buff2[i]);
			} else {
	...
	vector<void*> get_result(int&size) {
		...
		V2* res3 = static_cast<V2*>(malloc(sizeof(V2) * size));
		int z = 0;
		for(typename tr1::unordered_map<K, pair<V, V2> >::iterator i = map_.begin(); i != map_.end(); ++i) {
			res1[z] = i -> first;
...
public:
	void insert(void* key, void* key2, void* value, int rows) {
		...
		V* value_buff = static_cast<V*>(value);
		for(int i = 0; i < rows; ++i) {
			map_[make_pair(key_buff[i], key_buff2[i])] += value_buff[i];
		}
	}
	...
	vector<void*> get_result(int&size) {
		...
		for(typename tr1::unordered_map<pair<K, K2>, V>::iterator i = map_.begin(); i != map_.end(); ++i) {

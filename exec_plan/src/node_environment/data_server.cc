...
class RealDataServer : public Server {
 public:
  ...
  ~RealDataServer();
  map<int, int> column_sum_int_;
  map<int, double> column_sum_double_;
...
RealDataServer::RealDataServer(const vector<int> &column_types) {
	...
	cerr << "QUERRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRY " << query_size << endl;
...
RealDataServer::~RealDataServer() {
  ...
  for(map<int, int> :: iterator i = column_sum_int_.begin() ; i != column_sum_int_.end(); ++i) {
	  cerr << "Sum of column " << i -> first << " = " << i -> second << endl;
  }

  for(map<int, double> :: iterator i = column_sum_double_.begin() ; i != column_sum_double_.end(); ++i) {
	  cerr << "Sum of column " << i -> first << " = " << i -> second << endl;
  }
...
void RealDataServer::ConsumeDoubles(int column_index, int number,
                                  const double* d) {
  for (int i = 0; i < number; ++i) {
	  column_sum_double_[column_index] += d[i];
    //printf("C%d: %f\n", column_index, d[i]);
  }
}
...
void RealDataServer::ConsumeInts(int column_index, int number, const int32* d) {
  for (int i = 0; i < number; ++i) {
	  column_sum_int_[column_index] += d[i];
    //printf("C%d: %d\n", column_index, d[i]);
  }
}
...
void RealDataServer::ConsumeByteBools(int column_index, int number,
                                    const bool* d) {
  for (int i = 0; i < number; ++i) {
	  column_sum_int_[column_index] += static_cast<int>(d[i]);

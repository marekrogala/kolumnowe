...
private:
 ...
 public:
  ...
  NodeEnvironment(uint32 node_number,
                  NetworkInput* input,
                  std::vector<NetworkOutput*> outputs, int queryId)
  : node_number_(node_number),
    node_count_(outputs.size()),
    input_(input),
    outputs_(outputs), queryId_(queryId) {}
  ...
  virtual DataSourceInterface* OpenDataSourceFile(int source_file_id) {
    return new DataSource(CreateServer(queryId_));
  };

  virtual DataSinkInterface* OpenDataSink() {
	dupa_ = new DataSink(CreateServer(queryId_));
	return dupa_;
  };
...
NodeEnvironmentInterface* CreateNodeEnvironment(int argc, char** argv, int queryId) {
  ...
  return new NodeEnvironment(node_number, new NetworkInput(listening_port), outputs, queryId);
}
